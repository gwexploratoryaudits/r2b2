...
from r2b2.simulation.athena import AthenaOneRoundStoppingProb as AORSP 
...
def state_trial(state, alpha, sample_size):
    sim = AORSP(alpha, 1.0, election.contests[state], sample_size, sim_args={'description': 'Stopping Probability 90%: One round Athena with given sample size (from PV MATLAB)'}, reported_args={'name': state, 'description': '2016 Presidential'})
    sim.run(10000)
    return sim.analyze()
