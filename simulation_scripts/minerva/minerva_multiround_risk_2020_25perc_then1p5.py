...
def state_trial(state, alpha, sample_size):
    ...
    sim = MMRR(alpha,
               ...
               sample_mult=1.5,
               sim_args={'description': 'Multiround Minerva (25% then 1.5x)'},
               user='',
               pwd='',
               reported_args={
                   'name': state,
...
if __name__ == '__main__':
    for contest in election.contests.keys():
        ...
        if margin < 0.05:
            ...
            continue
        # we compute sample_size to achieve 25% stopping probability
        alpha = .1
        sprob = .25
        tmp_audit = Minerva(alpha, 1.0, election.contests[contest])
        first_sample_size = tmp_audit.next_sample_size(sprob)
        computed_risk = state_trial(contest, alpha, first_sample_size)
