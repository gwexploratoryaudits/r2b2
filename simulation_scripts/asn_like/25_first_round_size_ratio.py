...
if __name__ == '__main__':
    ...
    # MINERVA 1.0X 
    minerva1p0_points, margins = get_lists('minerva', 'Multiround Minerva (25% then 1.0x)')
    if minerva1p0_points is not None:
        minerva1p0_name = 'Minerva, 25% then 1.0x'
    ...
    # MINERVA 1.5X 
    minerva1p5_points, margins = get_lists('minerva', 'Multiround Minerva (25% then 1.5x)')
    if minerva1p5_points is not None:
        minerva1p5_name = 'Minerva, 25% then 1.5x'
    ...
    # SO BRAVO
    so_bravo_points, margins = get_lists('so_bravo', 'Multiround SO_BRAVO (25%)', max_rounds=100) # a few old 5 round so bravo sprob sims in db
    if so_bravo_points is not None:
        so_bravo_name = 'SO BRAVO, 25%'
    ...
    # EOR BRAVO
    eor_bravo_points, margins = get_lists('eor_bravo', 'Multiround EOR_BRAVO (25%) Corrected')
    if eor_bravo_points is not None:
        eor_bravo_name = 'EoR BRAVO, 25%'
    ...
    print('First round size ratios for',contest)
    eor_ratios = []
    for i in range(len(eor_bravo_points)):
        # i 1 0 is ith margin/state, 
        # 0 is sam nums, 
        # 0 is first round size
        eor_ratios.append(eor_bravo_points[i][0][0] / minerva1p0_points[i][0][0])
    print('average ratio (EoR / Minerva)', sum(eor_ratios) / len(eor_ratios))

    so_ratios = []
    for i in range(len(so_bravo_points)):
        # i 1 0 is ith margin/state, 
        # 0 is sam nums, 
        # 0 is first round size
        so_ratios.append(so_bravo_points[i][0][0] / minerva1p0_points[i][0][0])
    print('average ratio (EoR / Minerva)', sum(so_ratios) / len(so_ratios))
