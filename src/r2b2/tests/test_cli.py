...
def test_interactive_simple(monkeypatch, capsys):
    ...
    user_in = StringIO(
        '1000\n2\nA\n700\nB\n300\n1\nA\n0\n0.1\n0.2\n1\n200\n175\n')
    output_file = open(
        'src/r2b2/tests/data/cli_test_expected_out_interactive.txt', 'r')
...
def test_interactive_given_audit(monkeypatch, capsys):
    ...
    output_file = open(
        'src/r2b2/tests/data/cli_test_expected_out_interactive_given_audit.txt',
        'r')
    expected_out = output_file.read()
    monkeypatch.setattr('sys.stdin', user_in)
...
def test_interactive_given_contest(monkeypatch, capsys):
    ...
    output_file = open(
        'src/r2b2/tests/data/cli_test_expected_out_interactive_given_contest.txt',
        'r')
    expected_out = output_file.read()
    monkeypatch.setattr('sys.stdin', user_in)
    main([
        '-i', '--contest_file=src/r2b2/tests/data/single_contest_template.json'
    ])
    captured = capsys.readouterr()
    assert captured.out == expected_out
    output_file.close()
...
def test_interactive_given_both(monkeypatch, capsys):
    """Testng `r2b2 -i -a=brla -r=01 -m=0.2 --contest_file=/.../single_contest_template.json`
    ...
    user_in = StringIO('20\n19\n')
    output_file = open(
        'src/r2b2/tests/data/cli_test_expected_out_interactive_given_both.txt',
        'r')
    expected_out = output_file.read()
    monkeypatch.setattr('sys.stdin', user_in)
    main([
        '-i', '-a=brla', '-r=0.1', '-m=0.2',
        '--contest_file=src/r2b2/tests/data/single_contest_template.json'
    ])
    captured = capsys.readouterr()
    assert captured.out == expected_out
    output_file.close()
