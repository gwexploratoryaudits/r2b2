...
if __name__ == '__main__':
    ...
    loops_done = 0
    while remaining:
        # block_size is how many trials to read at once (few enough to fit into memory and not be super slow to read)
        block_size = 100
        trials = list(db.db.trials.find({'simulation': sprob_sim['_id']}).limit(block_size).skip(loops_done * block_size))
        if len(trials) == 0:
            break
        print('Got another ',str(block_size),'trials')
        print('Now doing the analysis on the',str(len(trials)),'trials')
        for trial in trials:
