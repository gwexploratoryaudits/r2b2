...
for cur_audit in audits:
    ...
    per_audit_results_so.update(cur_results)
...
for cur_audit in audits:
    ...
    plt.plot(ps,np.array(prop_misleadings),linestyle='--', marker=markers[i], color=colors[i], label='Providence (Virginia)')
    i += 1
...
for cur_audit in audits:
    ...
    print(prop_misleadings)
    plt.plot(ps,np.array(prop_misleadings),linestyle='--', marker='s', color=colors[i], label='Providence (pilot)')
    plt.yscale("log")
    i += 1
...
# so misleading
for cur_audit in audits:
    ps = per_audit_results_so[cur_audit]['ps']
    prop_misleadings = per_audit_results_so[cur_audit]['so_misleading_props']
    plt.plot(ps,
