...
if __name__ == '__main__':
    ...
    if minerva1p0_points is not None:
        ...
        for m in range(len(minerva1p0_points)):
            if m >= len(colors):
                break
            plt.plot(minerva1p0_points[m][0], minerva1p0_points[m][1], colors[globidx]+markers[globidx], label=minerva1p0_name+': Margin '+str(round(margins[m], 2)))
            globidx+=1
    ...
    if minerva1p5_points is not None:
        ...
        for m in range(len(minerva1p5_points)):
            if m >= len(colors):
                break
            plt.plot(minerva1p5_points[m][0], minerva1p5_points[m][1], colors[globidx]+markers[globidx], label=minerva1p5_name+': Margin '+str(round(margins[m], 2)))
            globidx+=1
    ...
    if so_bravo_points is not None:
        ...
        for m in range(len(so_bravo_points)):
            if m >= len(colors):
                break
            plt.plot(so_bravo_points[m][0], so_bravo_points[m][1], colors[globidx]+markers[globidx], label=so_bravo_name+': Margin '+str(round(margins[m], 2)))
            globidx+=1
    ...
    if eor_bravo_points is not None:
        ...
        for m in range(len(so_bravo_points)):
            if m >= len(colors):
                break
            plt.plot(eor_bravo_points[m][0], eor_bravo_points[m][1], colors[globidx]+markers[globidx], label=eor_bravo_name+': Margin '+str(round(margins[m], 2)))
            globidx+=1
    ...
    title = '[Texas] Probability of Stopping for Number of Ballots Sampled'
    plt.title(title)
