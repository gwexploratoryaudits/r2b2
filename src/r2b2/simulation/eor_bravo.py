...
class EOR_BRAVOMultiRoundStoppingProb(Simulation):
    """Simulate a multiround EOR_BRAVO audit.
    ...
    def __init__(self,
        ...
        super().__init__('eor_bravo', alpha, reported, 'reported', True, db_mode, db_host, db_port, db_name, user, pwd, *args, **kwargs)
        ...
        self.audit = EOR_BRAVO(self.alpha, 1.0, self.reported)
    ...
    def trial(self, seed):
        """Execute a multiround eor_bravo audit (using r2b2.eor_bravo.EOR_BRAVO)"""
...
class EOR_BRAVOMultiRoundRisk(Simulation):
    ...
    def __init__(self,
        ...
        if 'sim_args' in kwargs:
            ...
            kwargs['sim_args'] = {'max_rounds': max_rounds, 'sample_mult': sample_mult, 'sample_sprob': sample_sprob}
        super().__init__('eor_bravo', alpha, reported, 'tie', True, db_mode, db_host, db_port, db_name, user, pwd, *args, **kwargs)
        self.sample_size = sample_size
        self.sample_mult = sample_mult
        self.sample_sprob = sample_sprob
        self.max_rounds = max_rounds
...
class PerPrecinctEOR_BRAVOMultiRoundStoppingProb(Simulation):
    """Simulate a multi-round EOR_BRAVO audit.
    ...
    def analyze(self, verbose: bool = False, hist: bool = False):
        ...
        if not all_stopped:
            ...
            assert num_trials == len(totals_sampled)
            asn = sum(totals_sampled) / num_trials
        print('asn: '+str(asn)+'(printing from src/r2b2/simulation/eor_bravo.py)')
