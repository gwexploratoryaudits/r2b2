...
for cur_audit in audits:
    ...
    for p in ps:
        ...
        misleading_count = 0
        totals_sampled = []
        totals_distinct_precincts_sampled = []
        totals_fairfax_distinct_precincts_sampled = []
        totals_fairfax_ballots_sampled = []
        num_trials = 0
        all_stopped = True
        stopped = 0
        for trial in trials:
            ...
            misleading = False
            for i in range(len(sample['Hillary R. Clinton'])):
                if sample['Hillary R. Clinton'][i] < sample['Donald J. Trump'][i]:
                    misleading = True
                    break
            totals_sampled.append(trial['relevant_sample_size_sched'][-1])
            if misleading:
                misleading_count += 1
            # Do more, specifically, get the full fairfax-specific data so we can compute
            # quantiles
            num_trials += 1
            if trial['stop']:
                #totals_sampled.append(trial['relevant_sample_size_sched'][-1])
                ...
                totals_fairfax_distinct_precincts_sampled.append(sum(trial['fairfax_distinct_precincts']))
                totals_fairfax_ballots_sampled.append(sum(trial['fairfax_bals']))
            else:
                all_stopped = False


        # get standard deviation of total ballots sampled
        asn_std = statistics.pstdev(totals_sampled)
        print('got std of asn')
        ...
        analysis.update({'misleading_count':misleading_count,
                        ...
                        'totals_fairfax_distinct_precincts_sampled':list(totals_fairfax_distinct_precincts_sampled),
                        'totals_fairfax_ballots_sampled':list(totals_fairfax_ballots_sampled)})
